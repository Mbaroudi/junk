...
double pl2_ranker::score_one(const index::score_data& sd) 
{
    ...
    */
    ...
    double pl2_score = (tfn*log2(tfn/lambda) + (lambda + 1.0/12.0/tfn-tfn)*log2(exp(1.0)) + log2(2.0*pi*tfn)/2.0) / (tfn+1);

    return pl2_score; // Change 0 to the final score you calculated
