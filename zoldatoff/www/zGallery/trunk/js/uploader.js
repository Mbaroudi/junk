...
$(document).ready( function() {
	$.getJSON('php/upload.php', {action: 'listFiles'}, getList);
})
...
function getList(jsonData) {
	...
	var myHTML = '';
	var myLoader = '<img src="css/loader.gif"/>';
	
	for (var i=0; i<nFiles; i++){
		myHTML += '<tr>';
		myHTML += '<td><img id="img' + i + '" src="css/loader.gif" style="max-height:30px" /></td>';
		myHTML += '<td>' + jsonData.file_list[i].filename + '</td>';
		myHTML += '<td id="full' + i + '">' + myLoader + '</td>';
		myHTML += '<td id="norm' + i + '">' + myLoader + '</td>';
		myHTML += '<td id="small' + i + '">' + myLoader + '</td>';
		myHTML += '<td id="del' + i + '">' + myLoader + '</td>';
		myHTML += '<td id="sql' + i + '">' + myLoader + '</td>';
		myHTML += '<td><img id="yesno' + i + '" src="css/loader.gif" /></td>';
		myHTML += '</tr>';
	}
	$('#mainT').append(myHTML);
	/*
	for (var i = 0; i < nFiles; i++) {
		$.getJSON('php/upload.php', {filename: escape(jsonData.filelist[i].filename), filesize: "full"}, getStatus);
		$.getJSON('php/upload.php', {filename: escape(jsonData.filelist[i].filename), filesize: "norm"}, getStatus);
		$.getJSON('php/upload.php', {filename: escape(jsonData.filelist[i].filename), filesize: "thumb"}, getStatus);
	}
	*/
}
